...
- SSR渲染的组件里面，不能出现`v-if`、`v-show` 等指令，还有`自定义指令`也不能出现，这些指令都会操作DOM元素，在走SSR渲染的时候就会报错，SSR流程进行不下去。使`visibility: visible`和`visibility: hidden`，同时采用鼠标划入划出事件，来控制 dom的显示或隐藏。
